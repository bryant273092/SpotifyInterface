...
export class SpotifyService {
  ...
  private sendRequestToExpress(endpoint:string):Promise<any> {
    ...
    var promise_data = this.http.get(this.expressBaseUrl+endpoint).toPromise();
    return promise_data;
  }
  ...
  getAudioFeaturesForTrack(trackId:string):Promise<TrackFeature[]> {
    ...
    return this.sendRequestToExpress('/track-audio-features' +'/' + encodeURIComponent(trackId)).then((data)=>{
      return Object.keys(data).map((track, percentage) =>{
          return new TrackFeature(track, data[track])
